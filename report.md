...
*Figure 1: Distribution and Time Series Plot of Air Temperature.*
...
- Missing values: Forward filled, backward filled, median-imputed,   mode-imputed 
    ...
    - `Wet bulb temperature`, `Rain Intensity`, `Total Rain`, `Precipitation Type`, `Heading`: 75,887 missing -> 0 missing 
...
The cleaning process removed 13,426 rows. Missing numeric variables were imputed and missing measurement timestamp was filled using backward and forward fill. 
...
*Figure 2: Pattern Analysis showing monthly air temperature trend, daily humidity trend, and correlation matrix of key variables.*
...
- Training set size: **146,213 samples** (earlier data: April 2015 - May 2023)
- Test set size: **36,555 samples** (later data: May 2023 - November 2025)
...
|-------|----------|------|-----|
| Linear Regression | 0.4154 | 7.6939°C  | 5.7413°C  |
| XGBoost | 0.7441 | 5.0905°C  | 3.2852°C  |
...
*Figure 3: Final visualizations showing model performance comparison, predicted vs. actual, and top 10 feature importances.*
...
1. **Model Performance:** XGBoost achieves R² = 0.7441, indicating that 74.41% of the variance in air temperature can be explained by the features in the dataset
...
5. **Data Leakage Avoidance:** In order to avoid data leakage, we avoided creating derived and rolling features that included the target variable

The XGBoost model achieved an overall higher performance. The predicted vs.actual graph shows a scatter plot distributed around the perfect prediction line. There is some points that are scattered outside the line, but this is normal since there may be some extreme values when it comes to air temperature/weather prediction. 
...
1. **Data Quality:**
    - Very large dataset with many missing values. Imputation methods can introduce bias
...
3. **Feature Engineering:**
    - There were limited temporal features
...
### Conclusion
This project consisted of a 9-phase workflow that cleaned and analyzed the Chicago Beach Weather Sensors Data. Methods such as imputation and the IQR method were used to handle missing values and outliers. The plots showed temporal relationships for air temperature. From there we could see that air temperature varied by seasons. The models achieved good air temperature predictors with R² value of 0.7441 and RMSE value of 5.0905°C. Data leakage was prevented by avoiding creating derived features and rolling statistics that included the target variable. 
